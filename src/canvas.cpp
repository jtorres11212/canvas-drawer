...
void Canvas::end()
{
   if(check=2 && sizeof(dada)%2==0){//%2= modulo 2
      for(int i=0;i<sizeof(dada);i+=2){
         line(dada[i],dada[i+1]);
      }
   }
   else if(check=3 && sizeof(dada)%3==0){
      for(int i=0;i<sizeof(dada);i+=3){
...
void Canvas::triangle(dad p1, dad p2, dad p3){
   int mx=max(p1.x,p2.x,p3.x);
   int my=max(p1.y,p2.y,p3.y);
   int mnx=min(p1.x,p2.x,p3.x);
   int mny=min(p1.y,p2.y,p3.y);
